...
ui <-fluidPage(theme=shinytheme("paper"),
               navbarPage(
                 "Significant Earthquakes Shiny App",
                 navbarMenu("Plot Analysis",
                            tabPanel("Number of Earthquakes",
                                     #Sidebar panel for inputs
                                     sidebarLayout(
                                       sidebarPanel(
                                       h3("Significant Earthquakes in 1965-2016"),
                                    ...
                                    As you interact with the slider input you can notice as in the year 2011 there were 712 earthquakes, the highest valued in the period considered.",
                                         style = "font-family: 'times'; font-si16pt"), #endp
                            ...
                            tabPanel("Magnitude Density Plot", #Navbar2
                                     sidebarPanel(
                                       ...
                                       p("In this table I used a density plot for the numeric variable of Magnitude in order to show the probability density function, estimated on kernel density. 
                                         Thanks to the checkbox the user can interact with it according the different causes of seismic waves, given by the variable Type. **Rock Burst has only one observation with Magnitude 6.20", style = "font-family: 'times'; font-si16pt"),
                                       ...
                                       checkboxGroupInput("checkGroup1", label = h4("Type Multiple Selection"), 
                                                          ...
                                                          selected = "Earthquake"),#endslider2
                            ...
                            tabPanel("Depth", #Navbar3
                                     sidebarPanel(
                                       h3("Density of Depth"),
                                       p("As the previous plot the last one shows the density function of the numeric variable Depth.
                                         In the same way the user can interact with the checkbox according to the different causes of seismic waves, given by the variable Type"),
                                       
                                       #Input:
                                       checkboxGroupInput("checkGroup2", label = h4("Choose the Type:"), 
                                                          choices = c("Earthquake", "Explosion", "Nuclear Explosion","Rock Burst"),
                                                          selected = "Earthquake"), #endlsider3
                                       ), #endsidebarPanel3
                 ...
                 navbarMenu("Maps", #NavBar 2.1
                            ...
                            tabPanel("Earthquakes World Map", #navBar 2.2
                                     ...
                                     sidebarPanel(
                                       ...
                                       p("This last table provides the trend of earthquakes around the world. Also in this case the user can interact with the slider input choosing which years prefer.",
...
output$Plot3<- renderPlotly({
  ...
  geom_density()+scale_x_log10(breaks = seq.int(from = -10,to = 400,by = 20))+theme_classic()
  ggplotly(plotlydep)
...
output$EqMap<-renderPlot({
  ...
  #interactive_eq1<-subset(interactive_eq,(interactive_eq$Magnitude>input$magnitude[1] & interactive_eq$Magnitude<input$magnitude[2]))
